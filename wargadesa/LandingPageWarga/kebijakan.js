...
// https://tupaii.my.id/api/services?filters[email][$containsi]=test@gmail.com&populate=*
function loadTable() {
    const xhttp = new XMLHttpRequest();
    xhttp.open("GET", "https://tupaii.my.id/api/services?filters[email][$containsi]=tentsuriki@gmail.com&populate=*");
    ...
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        ...
        for (let object of objects.data) { 
          ...
          trHTML += '<td>'+attributes['applicant']+'</td>';

          // trHTML += '<td>'+attributes['start_date']+'</td>';
          trHTML += '<td>'+formatDate(buat)+'</td>';
                    // trHTML += '<td>'+formatDate(publish)+'</td>';
          
            trHTML += '<td>'+attributes['applicationType']+'</td>';
            trHTML += '<td>'+attributes['status']+'</td>';
